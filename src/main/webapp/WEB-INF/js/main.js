var url = 'http://localhost:8080/springmvc-xml-thymeleaf/subscribe';

//Vapid public key.
var applicationServerPublicKey = 'BENGlPcmp7oJo4QaLJXFgk5-e3W5_Rywr3KDF6KXjtbxfOlgk73az9w2Jfj7iZ8JgH9KgdH75s8hggrY0-7k-F8=';
...
$(document).ready(function () {
    ...
    $('#btnPushNotifications1').click(function (event) {
        if(isSubscribed){
           // console.log("Unsubscribing...");
            //unsubscribe();
        	notifyMessage();
...
function sendSubscriptionToServer(endpoint, key, auth) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({publicKey: encodedKey, auth: encodedAuth, notificationEndPoint: endpoint}),
...
function removeSubscriptionFromServer(endpoint) {
    $.ajax({
        type: 'POST',
        contentType : "application/json",
        url: '/springmvc-xml-thymeleaf/unsubscribe',
        data: JSON.stringify({notificationEndPoint: endpoint}),
        success: function (response) {
...
function notifyMessage(endpoint) {
	var message = {
			"title": "Tirumurugan1",
		    "message": "welcome friends",
		    "clickTarget": "https://new-storybook.herokuapp.com"	
	};
    $.ajax({
        type: 'POST',
        contentType : "application/json",
        url: '/springmvc-xml-thymeleaf/notify-all',
        data: JSON.stringify(message),
        success: function (response) {
            console.log('Notify successfully! ' + JSON.stringify(response));
        },
        dataType: 'json'
    });
}
